...
class MyUtils:
    ...
    def get_all_files(self, dir_):
        """Retourne la liste de tous les fichiers en objets Path"""
        path = Path(dir_)
...
def test_get_sub_dir():
    my_utils = MyUtils()
    a = my_utils.get_all_sub_directories('.')
...
def test_run_command_system():
    ...
    print(my_utils.run_command_system('ls'))

    # ls du dossiercourant
    print('\nls de /media/data/3D/projets/pymultilame/')
    print(my_utils.run_command_system('ls /media/data/3D/projets/pymultilame/'))
...
def test_get_all_files_list():
    ...
    print("\nListe des py dans", d)
    l = my_utils.get_all_files_list(d, "py")
    for f in l:
        print(f)

    print("list des txt dans", d)
    l = my_utils.get_all_files_list(d, "txt")
    for f in l:
        print(f)
...
if __name__ == "__main__":
    ...
    # # data = my_utils.do_read(fichier)
    # # print(data)

    my_utils.mkdir_in_home('grande_echelle_data')

    # Le dossier des data dans le home, c'est un objet Path
    data_dir = Path.home() / 'grande_echelle_data'
    print(f"Dossier des datas: {data_dir} type {type(data_dir)}")

    f = my_utils.get_all_files('.')
    print(f)
